...
#确保配置的合法性，符合设计空间要求
#tileb：batch size维度tile大小
#tilex/tiley：输入特征图的x/y维度tile大小 当前要求tilex=tiley
#目前不支持tiled memory ops
...
#针对transformer模型的多头注意力机制，获取对应的操作列表
...
#确保多头注意力机制中，存储操作对应的计算操作是最后一个，以保证数据一致性/计算完成才写内存  /store之前compute完成
...
#程序计数器更新逻辑，决定每个head的op指针何时前进/根据stall情况推进索引
	...
	while not scheduler.all_done():
		...
		if memory_op:
			...
			for head_idx in head_ids:
				...
				#发起memory操作
				# Perform memory operation store/load
